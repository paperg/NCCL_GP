...
static void printNodePaths(struct ncclTopoSystem* system, struct ncclTopoNode* node) {
  ...
  for (int t=0; t<NCCL_TOPO_NODE_TYPES; t++) {
    ...
    for (int n = 0; n<system->nodes[t].count; n++) {
      ...
      INFO(NCCL_GRAPH, "%s (%f) %s", line, node->paths[t][n].bw, topoPathTypeStr[node->paths[t][n].type]);
#else
...
compare:
  ...
  if (*p2p == 1) {
    ...
    if (g1 != g2 && ncclParamIgnoreDisabledP2p() != 2) {
      int indexes[3] = {-1,-1,-1};
      int verticeN = 0;
      // NCCLCHECK(ncclNvmlEnsureInitialized()); // Not Used
      ...
      for (int i=1; i < verticeN; i++) {
        ...
        // if (!good) {
        if(0) { // When simulating, all are enabled by default.
